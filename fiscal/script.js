...
adder.style.display = loggedin ? "block" : "none";
...
const ikoniceSekcije = document.querySelectorAll(".fa-ikonice > i");
let ikonicaSekcije = "";
ikoniceSekcije.forEach((ikonica) => {
  ikonica.onclick = () => {
    ikoniceSekcije.forEach((icon) => {
      icon.classList.remove("selected-icon");
    });
    ikonicaSekcije = ikonica.getAttribute("class");
    ikonica.classList.add("selected-icon");
  };
});

const dodavanjeSekcije = () => {
  const imeSekcije = document.getElementById("section-name");
  const bojaSekcije = document.getElementById("section-color");

  sekcije.push({
    ikonica: ikonicaSekcije,
    ime: imeSekcije.value,
    boja: bojaSekcije.value,
    artikli: [],
  });
  ...
  ikonicaSekcije = "";
  imeSekcije.value = "";
  bojaSekcije.value = "#000000";

  ikoniceSekcije.forEach((ikonica) => {
    ikonica.classList.remove("selected-icon");
  });
...
removeItems.style.display = loggedin ? "grid" : "none";
...
const removeArticle = (removeArticleItem) => {
  removeArticleItem.forEach((item) => {
    item.onclick = () => {
      const imeItema =
        item.parentElement.querySelector(".specificno-ime").innerText;
      sekcije.forEach((sekcija) => {
        sekcija.artikli.forEach((artikl) => {
          if (artikl.ime == imeItema) {
            const idTranslate = document
              .querySelector("#artikli-specificno > .visible")
              .getAttribute("id");
            const classTranslate = document
              .querySelector(".artikli > .selected")
              .getAttribute("class")
              .split(" ")[0];
            ...
            document
              .querySelector(`.artikli > .${classTranslate}`)
...
const dodavanjeCSS = () => {
  ...
  const specificniArtikliSvi = document.querySelectorAll(
    "#artikli-specificno > div"
  );

  for (let i = 0; i < artikli.length; i++) {
    artikli[i].style.setProperty("--article-accent", sekcije[i].boja);
    specificniArtikliSvi[i].style.setProperty(
      "--article-accent",
      sekcije[i].boja
    );
  }
...
const resetDOM = () => {
  ...
  artikli.forEach((artikl) => {
    artikl.onclick = () => {
      ...
      const specificniArtikliVisible = specificniArtikli.querySelector(
        "#" + imeArtikla
      );
...
const addArtiklSkrivac = () => {
  ...
  if (proveraSelektovanog !== null && loggedin == true) {
...
const dodavanjeArtiklaSekcija = () => {
  const potrebnaSekcija =
    artikliContainer.querySelector(".selected > span").innerText;
  sekcije.forEach((element) => {
    if (potrebnaSekcija == element.ime) {
      ...
      const specificniArtikliVisible = specificniArtikli.querySelector(
        "#" + potrebnaSekcija.toLowerCase().replace(" ", "-").replace(".", "")
      );
      specificniArtikliVisible.classList.add("visible");
    }
...
const dodavanjeArtiklaJednog = (element) => {
  ...
  element.artikli.push({
    ime: imeArtiklaDodavanje.value,
    cena: cenaArtiklaDodavanje.value,
  });
